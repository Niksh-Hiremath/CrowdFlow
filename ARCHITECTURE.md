...
- Visible scale/measurement labels when the plan provides them; otherwise all extracted lengths remain editable estimates.
...
5. Applies travel time, occupancy-dependent speed, node/edge storage, and throughput limits.
...
- occupancy/capacity ratio, speed factor, and inflow/outflow imbalance;
- queue-growth or stagnation reason, persistence duration, and severity;
...
The engine generates deterministic small-K alternatives, applies each candidate to a cloned state, advances baseline and rerouted states over the same horizon, and reports deltas in peak occupancy ratio, congestion exposure, and completed exits. A route is applicable only when this counterfactual passes the configured safety-improvement rule.

## Structured model advice

The deterministic engine creates a `FindingBundle` containing:

- the current graph and scenario IDs;
...
- graph-valid alternative route edges;
